...
export const loader = async ({ request }: LoaderArgs) => {
  ...
  const resp = await fetch(`http://localhost:3333/search?q=${searchStr}`);
  ...
  return <div>
    <Form method="get" action="/">
      <label>
        ...
        <select name="source">
          {
            SOURCES.map(({ id, text }) => <option value={id} key={id}>{text}</option>)
          }
        </select>
    ...
    <ul>
      {
        data.map(({ text, articleLink, articleTitle }: Row) => (
            <li className="row" key={`${text}${articleLink}`}>
              {text}<br/>[<small><a target="_blank" href={articleLink}>{articleTitle}</a></small>]
            </li>
        ))
      }
    </ul>
